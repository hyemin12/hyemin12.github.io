...
이 예제에서 Profile 컴포넌트로의 사용자 주입뿐만 아니라, withAuth 훅 자체도 useContext 훅을 통해 사용자를 의존성 주입으로 가져옵니다. 코드 어딘가에서 누군가가 컨텍스트에 사용자를 주입하는 프로바이더를 선언했을 것입니다. 이 사용자 인스턴스는 심지어 실행 중에 컨텍스트에서 사용자를 업데이트하여 변경할 수도 있습니다.
